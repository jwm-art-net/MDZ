...
static GtkWidget* bail_spin =               NULL;
...
void load_settings_cmd(void)
{
    if (do_image_info_load_dialog(img))
    {
        ...
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(bail_spin),
                                                        img->bail);
        gtk_spin_button_update(GTK_SPIN_BUTTON(bail_spin));
...
void gui_start_rendering(image_info* img)
{
    ...
    if (!img->j_pre)
    {
        ...
        gtk_spin_button_update( GTK_SPIN_BUTTON(bail_spin));
        img->bail = gtk_spin_button_get_value_as_int(
                                GTK_SPIN_BUTTON(bail_spin));
        if (opts.logfd)
        {
...
*/
    ...
    /* bail label */
    button = gtk_label_new("Bail");
    gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
    gtk_widget_show(button);

    /* bail spin-button */
    adj = gtk_adjustment_new((gfloat)img->bail,
                                MIN_BAILOUT,
                                MAX_BAILOUT, 1, 16, 0.0);
    bail_spin = gtk_spin_button_new(GTK_ADJUSTMENT(adj), 0.0, 0.0);
    gtk_widget_set_tooltip_text(bail_spin, "bailout value");
    gtk_box_pack_start(GTK_BOX(hbox), bail_spin, FALSE, FALSE, 0);
    gtk_widget_show(bail_spin);
